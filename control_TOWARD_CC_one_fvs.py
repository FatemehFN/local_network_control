...
coverage_control_file = os.path.join(RESULTS_DIR, 'la_one_fvs_percent_coverage_control_size.txt')
f1 = open(coverage_control_file, 'w')

# File to store model name and number of communities
communities_file = os.path.join(RESULTS_DIR, 'la_one_fvs_name_number_of_communities.txt')
...
for model_name in MODELS:
    ...
    # Calculate Feedback Vertex Set (FVS)
    fvs_list = FVS.FVS(G)
    fvs_size = len(fvs_list)
    ...
    # --- Identify Influential Nodes in Communities (from FVS) ---
    # This function selects one influential FVS member per community based on structural scores
    control_in_communities_dict = CLO.influential_nodes_in_communities_one_fvs_member(G, fvs_list)
    control_nodes = list(control_in_communities_dict.values()) # Extract the selected control nodes
    ...
    if not os.path.exists(boolean_model_path):
        ...
        # Write default values or skip this model completely in output files if an error occurs.
